...
type EngagementStageEnum =
  | "HIGH_INTENT"
  | "CONSIDERATION_STAGE"
  | "LOW_INTENT_TESTER";
type SelfSignupProductEnum =
  | "ACCOUNTS_PAYABLE"
  | "CORPORATE_CARDS"
...
function calculateLeadScore(leadData: LeadData): number {
  ...
  if (leadData.productInterest) {
    if (leadData.productInterest.length > 1) score += 10;
      ...
      leadData.productInterest.length === 1 &&
      leadData.productInterest[0] !== "I don't know"
    )
      score += 5;
  }
...
function determineSelfSignupProduct(leadData: LeadData): SelfSignupProductEnum {
  ...
  if (
    leadData.numberOfInvoices === ">500" ||
    leadData.numberOfInvoices === "201 - 500"
  ) {
  ...
  if (interests.length === 1) {
    ...
    if (interests[0] === "employee-reimbursement")
...
function determineNextStep(
  ...
  switch (currentStep) {
    ...
    case "registrationCountry":
      ...
      if (
        updateData.registrationCountry &&
        !SUPPORTED_COUNTRIES.has(updateData.registrationCountry.toUpperCase())
      ) {
    ...
    case "productInterests":
      ...
      if (
        updatedLeadData.numberOfEmployees === "1 - 10" ||
        updatedLeadData.numberOfEmployees === "11 - 25"
      ) {
 ...
 */
function calculateProgress(currentStep: BackendStepEnum): {
  stepNumber: number;
  totalSteps: number;
} {
...
export async function PATCH(
  request: NextRequest,
  { params }: { params: { leadId: string; currentFunnelStep: BackendStepEnum } }
) {
  ...
  try {
    const { leadId, currentFunnelStep } = params;
    ...
    if (!leadId || !currentFunnelStep) {
      return NextResponse.json(
        { error: "Missing leadId or currentFunnelStep in path" },
        { status: 400, headers: { ...corsHeaders } }
      );
    }

    // Parse request body
    const updateData: LeadUpdateDto = await request.json();
    ...
    const { nextStep, waitlistReason } = determineNextStep(
      currentFunnelStep,
      updateData,
      leadData
    );
    ...
    else if (nextStep === "demoBookingP0" || nextStep === "demoBookingP1")
      intent = "INTRO";
